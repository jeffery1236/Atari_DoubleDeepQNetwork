...
from skimage import random_noise
...
class MeanTeacherAgent(): # Double DQN with Mean Teacher semi-supervised learning strategy
    ...
    def store_memory(self, obs, aug_states, action, reward, new_obs, done):
        self.memories.store(obs, aug_states, action, reward, new_obs, done)
        self.mem_counter += 1

    def sample_memory(self):
        states, aug_states, actions, rewards, new_states, dones = self.memories.sample(self.mini_batchsize)

        states = T.tensor(states).to(self.target_network.device)
        teacher_states = T.tensor(aug_states).to(self.target_network.device)
